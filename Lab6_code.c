...
int opolicy=0; // starting with open page policy
int prev_row_rank,prev_row_channel,prev_row_row,prev_row_bank;
...
void schedule(int channel)
{
		...
		LL_FOREACH(write_queue_head[channel], wr_ptr)
		{
			if(wr_ptr->command_issuable)
			{
                if(wr_ptr->next_command == COL_WRITE_CMD){
                    //miss case
                    if(!(prev_row_bank == wr_ptr->dram_addr.bank && prev_row_rank == wr_ptr->dram_addr.rank && prev_row_row == wr_ptr->dram_addr.row && prev_row_channel == channel ))
                    {
                        counter = (counter+1);
						if(counter > 15) counter = 15;
                        if(counter > HI_TH) opolicy = 1;
                        else if(counter < LO_TH) opolicy = 0;
                    }
				   prev_row_bank = wr_ptr->dram_addr.bank;
				   prev_row_rank = wr_ptr->dram_addr.rank; 
				   prev_row_row = wr_ptr->dram_addr.row;
				   prev_row_channel= channel;
                }
	...
	if(!drain_writes[channel])
	{
		LL_FOREACH(read_queue_head[channel],rd_ptr)
		{
			...
			{
                if(rd_ptr->next_command == COL_WRITE_CMD){
                    if(!(prev_row_bank == rd_ptr->dram_addr.bank && prev_row_rank == rd_ptr->dram_addr.rank && prev_row_row == rd_ptr->dram_addr.row && prev_row_channel == channel )){
                        counter = (counter+1);
						if(counter > 15) counter = 15;
                        if(counter > HI_TH) opolicy = 1;
                        else if(counter < LO_TH) opolicy = 0;
                    }
				   prev_row_bank = rd_ptr->dram_addr.bank;
				   prev_row_rank = rd_ptr->dram_addr.rank; 
				   prev_row_row = rd_ptr->dram_addr.row;
				   prev_row_channel= channel;
                }
